...
The products of alignment are .sam files, and sorting then creates .bam files. These .bam files are inputs for variant calling.
...
This script produces a **.vcf** file *(Variant Call Format file)* that stores information about each genetic variant:
...
We will assess our .vcf file and conduct remaining steps in R.
...
We also filter using other thresholds: allelic balance, monomorphic SNPs, one SNP per RADtag (to minimize linkage disequilibrium bias), minor allele frequencies, and SNPs in Hardy-Weinberg disequilbrium if present across all sites.

**Filtered Dataset**

Sample size: **632** individuals

Variants: **936** biallelic sites (loci)

## 1.4 Clone correcting

Eelgrass reproduces both clonally and via flowering (rad, am I right?). Some meadows can be largely clonal (comprised of only a few clones, or what we will refer to as *multilocus lineages*). We need to remove any *duplicate multilocus lineages* in our dataset.
...
This step requires extracting allele frequencies from our genetic dataset that we finalized during clone correction.
